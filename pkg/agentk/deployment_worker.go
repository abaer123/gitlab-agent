...
// deploymentWorkerConfig holds configuration.
// Shared type to simplify configuration copying.
type deploymentWorkerConfig struct {
	log              *logrus.Entry
	projectId        string
	namespace        string
...
func (d *deploymentWorker) Run(ctx context.Context) {
	...
	s := synchronizer{
		ctx:                    ctx,
		eng:                    eng,
		deploymentWorkerConfig: d.deploymentWorkerConfig,
	}

	_ = wait.PollImmediateUntil(getObjectsToSynchronizeRetryPeriod, s.getObjectsToSynchronize, ctx.Done())
}
...
func (s *synchronizer) getObjectsToSynchronize() (bool /*done*/, error) {
...
func decodeObjectToSynchronize(obj *agentrpc.ObjectToSynchronize) (retObjs []*unstructured.Unstructured, retErr error) {
	...
	for {
		...
		decodedUnstructured := decodedRuntimeObj.(*unstructured.Unstructured)
		annotations := decodedUnstructured.GetAnnotations()
		if annotations == nil {
			annotations = make(map[string]string, 1)
		}
		annotations[managedObjectAnnotationName] = "managed" // TODO
		decodedUnstructured.SetAnnotations(annotations)
		unstructuredObjs = append(unstructuredObjs, decodedUnstructured)
	}
