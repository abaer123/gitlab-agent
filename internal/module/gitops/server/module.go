...
func (m *module) GetObjectsToSynchronize(req *rpc.ObjectsToSynchronizeRequest, server rpc.Gitops_GetObjectsToSynchronizeServer) error {
	...
	agentInfo, err, retErr := m.api.GetAgentInfo(ctx, l, agentMeta.Token, false)
	if retErr {
		return err
	}
	...
	l = l.With(logz.AgentId(agentInfo.Id), logz.ProjectId(req.ProjectId))
	return m.api.PollImmediateUntil(ctx, m.pollPeriod, m.maxConnectionAge, m.sendObjectsToSynchronize(l, req, server))
}
